...
(function (jQuery, window, document, undefined) {
	 ...
	 *
	 * Usage:
	 *      jQuery(element).countDown()
	 *
	 *      jQuery(element) is a valid <time> or any other HTML tag containing a text representation of a date/time
	 *      or duration remaining before a deadline expires.
	 *      If jQuery(element) is a <time> tag, the datetime attribute is checked first.
	 ...
	 *
	 *      If jQuery(element) is not a <time> tag, a new one is created and appended to jQuery(element).
	 *
	...
	jQuery.extend(CountDown.prototype, {
		init: function () {
			...
			if (this.element.attr("datetime")) {
				// Try to parse the datetime attribute first.
				this.endDate = this.parseEndDate(this.element.attr("datetime"));
			}
			if (this.endDate === undefined) {
				// Fallback on the text content.
			...
			if (this.element.is("time")) {
				...
				this.timeElement = jQuery("<time></time>");
		...
		markup: function () {
			...
			var html = [
				'<span class="item item-dd">',
				'<span class="dd"></span>',
				'<span class="label label-dd">',
				this.options.label_dd,
				"</span>",
				"</span>",
				'<span class="separator separator-dd">',
				this.options.separator_days,
				"</span>",
				'<span class="item item-hh">',
				'<span class="hh-1"></span>',
				'<span class="hh-2"></span>',
				'<span class="label label-hh">',
				this.options.label_hh,
				"</span>",
				"</span>",
				...
				'<span class="mm-1"></span>',
				'<span class="mm-2"></span>',
				'<span class="label label-mm">',
				this.options.label_mm,
				"</span>",
				"</span>",
				'<span class="separator">',
				this.options.separator,
				...
				'<span class="ss-1"></span>',
				'<span class="ss-2"></span>',
				'<span class="label label-ss">',
				this.options.label_ss,
				"</span>",
				"</span>",
			];
		...
		if (options === undefined || typeof options === "object") {
			return this.each(function () {
				if (!jQuery.data(this, "plugin_" + pluginName)) {
					jQuery.data(this, "plugin_" + pluginName, new CountDown(this, options));
				}
			});
		}

		// Allow any public function (i.e. a function whose name isn't 'init' or doesn't start with an underscore)
		// to be called via the jQuery plugin, e.g. jQuery(element).countDown('functionName', arg1, arg2).
